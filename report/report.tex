...
{\large Model-Based Deep Learning (361.2.2320) --- Spring 2025--2026\par}
\vspace{2cm}
\begin{tabular}{lll}
...
\end{tabular}
\vfill
Based on: X.~Chen, J.~Liu, Z.~Wang, W.~Yin,\\ \emph{``Hyperparameter Tuning is All You Need for LISTA,''} NeurIPS 2021.
\end{titlepage}

% ═════════════════════════════ Abstract ═════════════════════════════════════
\begin{abstract}
\noindent
Deep unrolling converts the iterations of a model-based optimization algorithm into the layers of a trainable network, combining the interpretability and sample efficiency of classical methods with the finite-depth performance of deep learning. This project studies the unrolling of the Iterative Shrinkage-Thresholding Algorithm (ISTA) for sparse recovery, spanning the full spectrum of learned flexibility: classical ISTA/FISTA (0 learned parameters), our three ISTA-derived scalar models (16--32 parameters), LISTA ($\sim$6M parameters), ALISTA (32 parameters), and HyperLISTA (3 tuned hyperparameters). On synthetic Gaussian sparse recovery, the model-based hierarchy holds strikingly: HyperLISTA attains $-61.4$\,dB NMSE with only 3 scalars, and our StepThresholdISTA (32 scalars) outperforms full LISTA with $187{,}000\times$ fewer parameters, revealing that the per-layer step-size schedule is the single most valuable component to learn. We further compare three training strategies for unfolded optimizers (end-to-end, deep supervision, greedy layer-wise) and weight tying. Finally, we stress-test all methods on pixel-domain compressed sensing of MNIST and Fashion-MNIST images, where the ranking inverts: methods built on an i.i.d.\ Gaussian-sparse prior collapse, while data-driven LISTA remains robust. The two regimes together illustrate a central lesson of model-based deep learning: structure wins when the model matches the data, and a wrong prior is worse than no prior.
\end{abstract}
...
This project implements and evaluates this full spectrum, asks \emph{which components of ISTA are actually worth learning}, compares the training strategies for unfolded optimizers discussed in class, and stress-tests the resulting models on real image data under compressed sensing.
...
\emph{Part B (images):} pixel-domain compressed sensing of MNIST and Fashion-MNIST: the image is flattened to $x\in[0,1]^{784}$ and measured as $b = Ax$ with measurement ratio $m/784 \in \{0.25, 0.5\}$. Both datasets are naturally sparse in the pixel domain (MNIST $\approx 78\%$, Fashion-MNIST $\approx 51.5\%$ exact zeros), so no transform is needed. Metrics: NMSE, PSNR and SSIM. Benchmarks in both parts: ISTA and FISTA (classical), LISTA (fully learned), ALISTA and HyperLISTA (structured).
...
N.~Shlezinger, J.~Whang, Y.~C.~Eldar, and A.~G.~Dimakis,
``Model-based deep learning,''
\emph{Proceedings of the IEEE}, vol.~111, no.~5, pp.~465--499, 2023.
